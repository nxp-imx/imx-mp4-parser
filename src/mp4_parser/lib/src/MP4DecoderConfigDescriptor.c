...
static MP4Err createFromInputStream(struct MP4DescriptorRecord* s, MP4InputStreamPtr inputStream) {
    ...
    DEBUG_SPRINTF("upstream = %d", (int)self->upstream);
    DEBUG_SPRINTF("streamType = %d", (int)self->streamType);
    DEBUG_SPRINTF("reserved = %d", (int)(val & 1));
    ...
    DEBUG_SPRINTF("bufferSizeDB = %d", (int)self->bufferSizeDB);
