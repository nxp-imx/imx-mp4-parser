...
*/
/*
    $Id: MP4DefaultCommand.c,v 1.2 2000/05/17 08:01:28 francesc Exp $
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static MP4Err createFromInputStream(struct MP4DescriptorRecord* s, MP4InputStreamPtr inputStream) {
    ...
    self->data = (char*)MP4LocalCalloc(1, self->dataLength);
...
MP4Err MP4CreateDefaultCommand(u32 tag, u32 size, u32 bytesRead, MP4DescriptorPtr* outDesc) {
    ...
    sprintf(msgBuf, "(some command with tag 0x%02x and size %d)", (unsigned int)tag, (int)size);
    self->name = (char*)MP4LocalCalloc(1, strlen(msgBuf) + 1);
