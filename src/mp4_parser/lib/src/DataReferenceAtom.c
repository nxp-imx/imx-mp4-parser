...
*/
/*
    $Id: DataReferenceAtom.c,v 1.13 1999/04/18 23:53:55 mc Exp $
*/
...
static u32 getEntryCount(struct MP4DataReferenceAtom* self) {
    u32 entryCount = 0;
    (void)MP4GetListEntryCount(self->atomList, &entryCount);
    return entryCount;
}
...
static MP4Err getEntry(struct MP4DataReferenceAtom* self, u32 dataReferenceIndex,
    ...
    err = MP4NoErr;
    if ((dataReferenceIndex < 1) || (outEntry == NULL) ||
        (dataReferenceIndex > getEntryCount(self)))
        BAILWITHERROR(MP4BadParamErr)
...
static MP4Err getOffset(struct MP4DataReferenceAtom* self, u32 dataReferenceIndex, u32* outOffset) {
    MP4DataEntryAtomPtr entry;
    MP4Err err;

    err = MP4NoErr;
    if ((dataReferenceIndex < 1) || (outOffset == NULL) ||
        (dataReferenceIndex > getEntryCount(self)))
        BAILWITHERROR(MP4BadParamErr)
    err = MP4GetListEntry(self->atomList, dataReferenceIndex - 1, (char**)&entry);
    if (err)
...
MP4Err MP4CreateDataReferenceAtom(MP4DataReferenceAtomPtr* outAtom) {
    ...
    self = (MP4DataReferenceAtomPtr)MP4LocalCalloc(1, sizeof(MP4DataReferenceAtom));
    if (self == NULL)
