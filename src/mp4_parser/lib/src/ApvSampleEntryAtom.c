...
static void destroy(MP4AtomPtr s) {
    MP4Err err;
    MP4ApvSampleEntryAtomPtr self;
    err = MP4NoErr;
    self = (MP4ApvSampleEntryAtomPtr)s;
    if (self == NULL)
        BAILWITHERROR(MP4BadParamErr)
    if (self->APVCAtomPtr) {
        self->APVCAtomPtr->destroy(self->APVCAtomPtr);
        self->APVCAtomPtr = NULL;
    }
    if (self->colrAtomPtr) {
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream) {
    MP4Err err;
    MP4ApvSampleEntryAtomPtr self = (MP4ApvSampleEntryAtomPtr)s;
    ...
    while (self->bytesRead + 8 < self->size) {
        ...
        if (MP4ApvCAtomType == outAtom->type &&
            self->APVCAtomPtr == NULL) { /* the 'apvC' child atom is found */
            self->APVCAtomPtr = outAtom;
...
MP4Err MP4CreateApvSampleEntryAtom(MP4ApvSampleEntryAtomPtr* outAtom) {
    ...
    self = (MP4ApvSampleEntryAtomPtr)MP4LocalCalloc(1, sizeof(MP4ApvSampleEntryAtom));
    TESTMALLOC(self)

    err = MP4CreateBaseAtom((MP4AtomPtr)self);
    if (err)
        goto bail;
    self->type = MP4ApvSampleEntryAtomType;
    self->name = "APV sample entry";
