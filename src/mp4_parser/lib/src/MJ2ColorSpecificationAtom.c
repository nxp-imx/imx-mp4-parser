...
*/
/*
    $Id: MJ2ColorSpecificationAtom.c,v 1.0 2000/10/06 10:02:00 tm Exp $
*/
...
static ISOErr createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream) {
    ...
    if (self->method == MJ2EnumeratedColorSpace) {
        ...
        self->profileSize = (u32)(self->size - self->bytesRead);
        self->profile = (char*)MP4LocalMalloc(self->profileSize);
...
ISOErr MJ2CreateColorSpecificationAtom(MJ2ColorSpecificationAtomPtr* outAtom) {
    ...
    self = (MJ2ColorSpecificationAtomPtr)MP4LocalCalloc(1, sizeof(MJ2ColorSpecificationAtom));
    TESTMALLOC(self);
