...
*/
/*
    $Id: TrackReferenceTypeAtom.c,v 1.2 2000/05/17 08:01:32 francesc Exp $
*/
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream) {
    ...
    self->trackIDs = (u32*)MP4LocalCalloc(1, bytesToRead);
...
static MP4Err addTrackID(struct MP4TrackReferenceTypeAtom* self, u32 trackID) {
    ...
    for (i = 0; i < self->trackIDCount; i++) {
        if (self->trackIDs[i] == trackID)
            return err;
    }

    self->trackIDs = (u32*)MP4LocalReAlloc(self->trackIDs, (self->trackIDCount + 1) * sizeof(u32));
    //	printf("Realloc in Track ID\n");
    TESTMALLOC(self->trackIDs);
...
MP4Err MP4CreateTrackReferenceTypeAtom(u32 atomType, MP4TrackReferenceTypeAtomPtr* outAtom) {
    ...
    self = (MP4TrackReferenceTypeAtomPtr)MP4LocalCalloc(1, sizeof(MP4TrackReferenceTypeAtom));
    TESTMALLOC(self)
