...
static void destroy(MP4AtomPtr s) {
    MP4Err err;
    AdpcmSampleEntryAtomPtr self;
    err = MP4NoErr;
    self = (AdpcmSampleEntryAtomPtr)s;
    if (self == NULL)
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream) {
    ...
    AdpcmSampleEntryAtomPtr self = (AdpcmSampleEntryAtomPtr)s;
    ...
    if (self->version) { /* QT version is 1, usually 'sowt' & 'twos' is version 1, 'raw ' is version
        ...
        self->bitsPerSample = 4;  // hard code for adpcm
    }

    self->decoderSpecificInfoSize = (u32)(self->size - self->bytesRead);
    // printf("IMA ADPCM version %d, channels %d, sample size %d, sample Rate %d, decoder specific
...
/* Create ADPCM sample entry atom */
MP4Err MP4CreateAdpcmSampleEntryAtom(AdpcmSampleEntryAtomPtr* outAtom, u32 type) {
    MP4Err err;
    AdpcmSampleEntryAtomPtr self;

    self = (AdpcmSampleEntryAtomPtr)MP4LocalCalloc(1, sizeof(AdpcmSampleEntryAtom));
    TESTMALLOC(self);
    ...
    self->name = "adpcm audio sample entry";
