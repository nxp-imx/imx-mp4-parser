...
*/
/*
    $Id: ChunkLargeOffsetAtom.c,v 1.2 2000/05/17 08:01:26 francesc Exp $
*/
...
static void destroy(MP4AtomPtr s) {
    MP4Err err;
    MP4ChunkLargeOffsetAtomPtr self;
    err = MP4NoErr;
    self = (MP4ChunkLargeOffsetAtomPtr)s;
    if (self == NULL)
...
{
	MP4Err err;
	MP4ChunkLargeOffsetAtomPtr self = (MP4ChunkLargeOffsetAtomPtr) s;
	
	err = MP4NoErr;
	if ( (self == NULL) || (outOffset == NULL) || (chunkNumber == 0) || (chunkNumber > self->entryCount) )
		BAILWITHERROR( MP4BadParamErr )
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
	...
	self->offsets = (u64 *) MP4LocalCalloc( self->entryCount, sizeof(u64) );
	TESTMALLOC( self->offsets );
   
    GETBYTES_V_MSG( (self->entryCount*sizeof(u64)), self->offsets, NULL);
    ...
    for ( entries = 0, p = self->offsets; entries < self->entryCount; entries++, p++ )
	{
        ...
        offset = ((u64)pByte[0] <<  56) |  ((u64)pByte[1] <<  48) |  ((u64)pByte[2] <<  40) |  ((u64)pByte[3] <<  32) | \
                 ((u64)pByte[4] <<  24) |  ((u64)pByte[5] <<  16) |  ((u64)pByte[6] <<  8) |  (u64)pByte[7];  
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream) {
    ...
    self->offsets = (u64*)MP4LocalCalloc(entry_cnt, sizeof(u64));
    TESTMALLOC(self->offsets);
...
MP4Err MP4CreateChunkLargeOffsetAtom(MP4ChunkLargeOffsetAtomPtr* outAtom) {
    ...
    self = (MP4ChunkLargeOffsetAtomPtr)MP4LocalCalloc(1, sizeof(MP4ChunkLargeOffsetAtom));
    TESTMALLOC(self);
