...
*/
/*
    $Id: TrackHeaderAtom.c,v 1.2 2000/05/17 08:01:31 francesc Exp $
*/
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream) {
    ...
    if (self->version == 1) {
        ...
        DEBUG_SPRINTF("duration = %Lf", self->duration);
    } else {
        u32 val;
    ...
    self->qt_trackWidth = (self->qt_trackWidth >> 16);
    self->qt_trackHeight = (self->qt_trackHeight >> 16);
bail:
    TEST_RETURN(err);
...
MP4Err MP4CreateTrackHeaderAtom(MP4TrackHeaderAtomPtr* outAtom) {
    ...
    self = (MP4TrackHeaderAtomPtr)MP4LocalCalloc(1, sizeof(MP4TrackHeaderAtom));
    TESTMALLOC(self)
    ...
    self->flags = 1; /* track is enabled, by default */
