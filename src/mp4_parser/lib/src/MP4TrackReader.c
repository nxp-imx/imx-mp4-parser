...
*/
/*
    $Id: MP4TrackReader.c,v 1.3 2000/04/21 14:42:42 francesc Exp $
*/
...
MP4Err MP4DisposeOrdinaryTrackReader(MP4TrackReader theReader);
...
MP4Err MP4CreateTrackReader(MP4Track theTrack, MP4TrackReader* outReader) {
    ...
    if (err)
        goto bail;
    switch (handlerType) {
        case MP4ObjectDescriptorHandlerType: /* not support reading such tracks */
            reader = NULL;
...
/*********** ****************************************
 *Fucntion to get the trak reader postion set. Ranjeet
 ******************************************************/
MP4Err MP4GetTrackReader(MP4Track theTrack, u32 FrameNumber, MP4TrackReader* outReader) {
    ...
    if (err)
        goto bail;
    err = MP4GetOrdinaryTrackReader(theMovie, FrameNumber, theTrack, &reader);
    if (err)
        goto bail;
...
MP4Err MP4TrackReaderGetNextAccessUnit(MP4TrackReader theReader, MP4Handle outAccessUnit,
...
MP4Err MP4TrackReaderGetNextAccessUnitWithDuration(MP4TrackReader theReader,
                                                   MP4Handle outAccessUnit, u32* outSize,
                                                   u32* outSampleFlags, u64* outCTS, u64* outDTS,
                                                   u32* outDuration, u32* max_chunk_offset,
    ...
    if ((theReader == 0) || (outAccessUnit == 0))
        BAILWITHERROR(MP4BadParamErr)
    reader = (MP4TrackReaderPtr)theReader;
    //*outSize = 0; /* this is the buffer size as input, need to tell the internal */
    reader->chunk_offset = max_chunk_offset;
    reader->odsm_offset = odsm_offset;
    reader->sdsm_offset = sdsm_offset;
...
MP4Err MP4TrackReaderGetNextPacket(MP4TrackReader theReader, MP4Handle outSample, u32* outSize) {
    MP4Err err;
    MP4TrackReaderPtr reader;
...
MP4Err MP4DisposeTrackReader(MP4TrackReader theReader) {
    ...
    reader = (MP4TrackReaderPtr)theReader;
    /* Guido : inserted to clean-up resources */
    err = MP4DisposeOrdinaryTrackReader((MP4TrackReader)reader);
    if (err)
        goto bail;
