...
*/
/*
    $Id: MP4ESDescriptor.c,v 1.12 2000/06/11 07:38:50 mc Exp $
*/
#include <stdlib.h>
#include <string.h>
...
static MP4Err createFromInputStream(struct MP4DescriptorRecord* s, MP4InputStreamPtr inputStream) {
    ...
    // MP4MSG("ES descriptor size %ld, bytes read %ld\n", self->size, self->bytesRead);
    ...
    DEBUG_SPRINTF("streamDependenceFlag = %d", (int)streamDependenceFlag);
    DEBUG_SPRINTF("urlflag = %d", (int)urlflag);
    DEBUG_SPRINTF("streamPriority = %d", (int)self->streamPriority);
    DEBUG_SPRINTF("OCRStreamFlag = %d", (int)ocrflag);

    if (urlflag) {
        GET8(URLStringLength);
        self->URLString = (char*)MP4LocalCalloc(1, self->URLStringLength);
    ...
    while (self->bytesRead < self->size) {
        MP4DescriptorPtr desc;
        err = MP4ParseDescriptor(inputStream, self->size - self->bytesRead, &desc);
        if (err)
            goto bail;
        if (!desc) {
            MP4MSG("Error happned in parsing ESDescriptor child descriptors. Stop parsing!\n");
            goto bail;
        }
        // MP4MSG("ESDescriptor got child descriptor tag %ld\n", desc->tag);
        switch (desc->tag) {
            case MP4DecoderConfigDescriptorTag:
                if (self->decoderConfig)
                    BAILWITHERROR(MP4InvalidMediaErr)
