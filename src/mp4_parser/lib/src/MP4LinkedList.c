...
MP4Err MP4MakeLinkedList(MP4LinkedList* outList) {
    ...
    if (sizeof(MP4List) == 0)
        return err;
    newList = (MP4LinkedList)MP4LocalCalloc(1, sizeof(MP4List));
    TESTMALLOC(newList)
...
MP4Err MP4PrependListEntry(MP4LinkedList list, void* item) {
    ...
    err = MP4NoErr;
    entry = (MP4ListItemPtr)MP4LocalCalloc(1, sizeof(MP4ListItem));
    TESTMALLOC(entry);
    entry->data = item;
    if (list->head == NULL) {
        list->entryCount = 1;
...
MP4Err MP4RemoveListEntry(MP4LinkedList list, u32 itemNumber, char** outItem) {
    MP4Err err;
    MP4ListItemPtr entry;
    u32 i = 0;
    err = MP4NoErr;
    ...
    if (itemNumber == 0) {
        if (outItem != NULL)
            *outItem = entry->data;
        list->head = entry->link;
        if (list->tail == entry)
            list->tail = NULL;
        MP4LocalFree(entry);
    } else {
        MP4ListItemPtr preEntry = list->head;
        MP4ListItemPtr delEntry;
        while (i < itemNumber - 1) {
            preEntry = preEntry->link;
            i++;
        }
        delEntry = preEntry->link;
        preEntry->link = delEntry->link;
        if (list->tail == delEntry)
            list->tail = NULL;
        if (outItem != NULL)
            *outItem = delEntry->data;
        MP4LocalFree(delEntry);
    }
