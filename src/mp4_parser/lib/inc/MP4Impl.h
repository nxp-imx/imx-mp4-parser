...
*/
/*
    $Id: MP4Impl.h,v 1.24 1999/07/14 07:50:10 mc Exp $
*/
...
#if defined(__WINCE)
//#define MP4MSG(cond, fmt, ...) DEBUGMSG(cond, _T(fmt), __VA_ARGS__))
#define MP4MSG(fmt, ...) DEBUGMSG(1, (_T(fmt), __VA_ARGS__))

#elif defined(WIN32)
#define MP4MSG(fmt, ...) DEBUGMSG(1, (fmt, __VA_ARGS__))

#else /* linux platfom */
#ifdef DEBUG
#ifdef ANDROID
#include "android/log.h"
#define MP4MSG(...) __android_log_print(ANDROID_LOG_INFO, "MP4Parser", __VA_ARGS__)
#else
#define MP4MSG printf
#endif
#else
#define MP4MSG(fmt...)
#endif
...
#define SKIPBYTES_FORWARD(length)          \
    ...
    inputStream->file_offset += (length);  \
...
#define GETBYTES_V(length, varname)                                      \
    err = inputStream->readData(inputStream, length, varname, #varname); \
    if (err)                                                             \
        goto bail;                                                       \
    self->bytesRead += length
...
#define GET_ATOM_SIZE(descName) (self->descName->size)
#define GET_ATOM_TYPE(descName) (self->descName->type)
...
/* serial all children atoms in the children list */
...
#define SETUP_BASE_DESCRIPTOR(classname)                         \
    ...
    classname##Ptr self;                                         \
    err = MP4NoErr;                                              \
    self = (classname##Ptr)MP4LocalCalloc(1, sizeof(classname)); \
    TESTMALLOC(self)                                             \
    self->tag = tag;                                             \
    self->size = size;                                           \
    self->name = #classname;                                     \
...
#define MP4LocalFileOpen(FileName, ModeToOpen, appContext) \
    g_streamOps.Open((const uint8*)(FileName), ModeToOpen, appContext)

#define MP4LocalGetCurrentFilePos(fileHandle, appContext) \
    g_streamOps.Tell((FslFileHandle)(fileHandle), appContext)

#define MP4LocalSeekFile(fileHandle, offset, origin, appContext)                       \
    {                                                                                  \
        if (g_streamOps.Seek((FslFileHandle)(fileHandle), offset, origin, appContext)) \
            BAILWITHERROR(PARSER_SEEK_ERROR)                                           \
    }

#define MP4LocalFileClose(fileHandle, appContext) \
    g_streamOps.Close((FslFileHandle)(fileHandle), appContext)

#define MP4LocalReadFile(buffer, size, number, fileHandle, appContext)                          \
    {                                                                                           \
        uint32 sizeToRead;                                                                      \
        uint32 sizGot;                                                                          \
        sizeToRead = (size) * (number);                                                         \
        sizGot = g_streamOps.Read((FslFileHandle)(fileHandle), buffer, sizeToRead, appContext); \
        if (sizGot == 0)                                                                        \
            BAILWITHERROR(PARSER_EOS)                                                           \
        else if (sizGot != sizeToRead)                                                          \
            BAILWITHERROR(PARSER_READ_ERROR)                                                    \
    }

#define MP4LocalFileSize(fileHandle, appContext) \
    g_streamOps.Size((FslFileHandle)(fileHandle), appContext)

#define MP4LocalCheckAvailableBytes(fileHandle, bytesRequested, appContext) \
    g_streamOps.CheckAvailableBytes((FslFileHandle)(fileHandle), bytesRequested, appContext)

#define MP4LocalGetFlag(fileHandle, appContext) \
    (g_streamOps.GetFlag) ? g_streamOps.GetFlag((FslFileHandle)(fileHandle), appContext) : 0

#define MP4LocalRequestBuffer(trackNum, size, bufContext, parserContext) \
    g_outputBufferOps.RequestBuffer(trackNum, size, bufContext, parserContext)

#define MP4LocalReleaseBuffer(trackNum, buffer, bufContext, parserContext) \
    g_outputBufferOps.ReleaseBuffer(trackNum, buffer, bufContext, parserContext)
